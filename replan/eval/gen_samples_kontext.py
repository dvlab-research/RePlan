...
class KontextGenConfig:
    ...
    output_dir: str = "output/kontext_gen"
    seed: int = 42
...
def main(conf: KontextGenConfig):
    ...
    # Load pipeline
    print(f"Loading model {conf.flux_model_name}...")
    pipe = FluxKontextPipeline.from_pretrained(conf.flux_model_name, torch_dtype=torch_dtype)
    pipe.to('cuda')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate images using FluxKontextPipeline from a config file.")
