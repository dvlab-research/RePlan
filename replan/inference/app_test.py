...
def generate_default_split_rules(num_regions, has_image_prompt=True):
    ...
    if has_image_prompt: 
        logical_components.append('Image Prompt') # <--- 只有 Qwen 才加这个
...
<head>
    ...
    <style>
        ...
        /* --- Attention Matrix Styles (Updated for Split Groups) --- */
        .attn-container { border: 1px solid #374151; border-radius: 6px; background: #111827; overflow: hidden; margin-top: 5px; }
        .attn-header { background: #1f2937; padding: 6px 10px; font-size: 11px; color: #9ca3af; display: flex; justify-content: space-between; border-bottom: 1px solid #374151; }
        .attn-table-wrap { overflow: auto; max-height: 300px; padding: 0; } /* 移除 padding 增加可视面积 */
        .attn-table { border-collapse: separate; border-spacing: 0; width: max-content; font-family: ui-monospace, monospace; font-size: 10px; } /* width: max-content 确保不换行 */
        ...
        .attn-x { color: #ef4444; font-weight: bold; font-size: 12px; opacity: 0.3; } /*稍微调低叉号亮度，减少视觉干扰*/
        ...
        <div class="col col-right">
            ...
            <div class="attn-container">
                <div class="attn-header">
                    <span>Matrix (Query → Key)</span>
                    <span style="cursor:pointer; text-decoration:underline" onclick="window.AttnManager.refresh(true)">Reset Default</span>
                </div>
                <div id="attn-matrix-root" class="attn-table-wrap">
                    </div>
            </div>
            ...
            <div style="margin-bottom:10px; background:#f9fafb; padding:10px; border-radius:6px; border:1px solid #e5e7eb;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; font-size:12px; color:#374151;">
                    <div style="display:flex; flex-direction:column;">
                        <span style="font-weight:600;">Rule Switch Step</span>
                        <span style="font-size:9px; color:#9ca3af;">Custom Rules ends here</span>
                    </div>
                    <input type="number" id="switch-step-input" min="0" max="100" value="10" style="width:60px; padding:4px; border:1px solid #d1d5db; border-radius:4px; text-align:right;">
                </div>
            </div>
...
window.AttnManager = {
    ...
    // 辅助：判断是否属于图像类组件 (BBox 或 Background)
    ...
    checkDefaultRule: function(q, k, hasImagePrompt) {
        ...
        // 3. Image Internal (Noise/Image BBox/BG 互看)
        if (qIsImg && kIsImg) return true;

        // 4. Cross Attention (Image <-> Main Prompt)
        if ((qIsImg && kGroup === 'Main Prompt') || (qGroup === 'Main Prompt' && kIsImg)) return true;

        // 5. Hint 逻辑
        if (qGroup.startsWith('Hint')) {
            // Hint attends to Self (covered by step 1 if names match)
            // Hint attends to all Image components
            if (kIsImg) return true;
        }
        if (kGroup.startsWith('Hint')) {
             // 只有对应的 BBox 能看 Hint (Group level match)
             // qGroup="BBox 1", kGroup="Hint 1"
    ...
    toggle: function(q, k) {
        ...
        // 强制刷新一下UI显示（不需要完整Render，如果为了性能可以只更class，但这里数据量小直接Render更安全）
        this.render(); 
    },
...
runEdit: async function() {
        ...
        // --- 新增：获取 switch step 并添加到 FormData ---
...
def main():
    ...
    def mount_custom_routes(app: FastAPI):
        ...
        async def serve_ui():
            ...
            # 2. 注入 Image Prompt 配置
            # 判断逻辑：只有 qwen 有 Image Prompt，Flux 没有
        ...
        async def api_edit(
            ...
            attention_rules: str = Form(None), # 前端传来的 Custom Matrix
            attention_switch_step: int = Form(10) # 前端传来的 Switch Step
        ):
            ...
            # 1. 解析前端传来的 Custom Rules (JSON -> Tuple Key Dict)

            # 1. 判断是否包含 Image Prompt
            current_type = getattr(pipeline, "pipeline_type", "flux")
            ...
            # 2. 解析 XML 获取 Region 数量 (用于生成 Default Rules)
            num_regions = 0
            try:
            ...
            # 3. 生成 Default Rules (Split Version)
            default_rules_map = generate_default_split_rules(num_regions, has_image_prompt=use_image_prompt)

            # 4. 构建 Step-based Rules Dictionary {0: rules, 1: rules ...}
            #    这是核心修改：将 Switch 逻辑转换为 Step 字典
            step_attention_dict = {}
            
            # 确保 switch_step 不越界
            switch_at = max(0, min(attention_switch_step, num_inference_steps))
            
            for step in range(num_inference_steps):
                if step < switch_at:
                    # 前期使用用户自定义规则
                    # 注意：如果 custom_rules_map 为空，最好回退到 default
            ...
            with gpu_lock:
                try:
                    ...
                    print(f"[API Edit] Processing | Steps: {num_inference_steps} | Switch Logic Applied at Step {switch_at}")
                    ...
                    res = pipeline.region_edit_with_attention(
                        ...
                        # 移除 attention_switch_step 参数，因为逻辑已经内化到上面的字典里了
                        # ----------------
